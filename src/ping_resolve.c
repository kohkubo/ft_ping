...
int resolve_hostname(PingContext *ctx, const char *hostname) {
    ...
    if (inet_pton(AF_INET, hostname, &sin->sin_addr) == 1) {
        ...
        ret = snprintf(ctx->dest_hostname, sizeof(ctx->dest_hostname), "%s", hostname);
    ...
    if (res->ai_addr->sa_family != AF_INET || res->ai_addrlen != sizeof(struct sockaddr_in)) {
    ...
    // IPアドレス文字列を安全に作成
    if (inet_ntop(AF_INET, &sin->sin_addr, ctx->dest_ip, sizeof(ctx->dest_ip)) == NULL) {
    ...
    // ホスト名を安全にコピー
    int ret = snprintf(ctx->dest_hostname, sizeof(ctx->dest_hostname), "%s", hostname);
    if ((size_t)ret >= sizeof(ctx->dest_hostname) || ret < 0) {
        printf("ping: hostname string too long\n");
        freeaddrinfo(res);
        return -1;
